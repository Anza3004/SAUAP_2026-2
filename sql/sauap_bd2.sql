...
-- Dump completed on 2026-09-20  0:28:27
...
CREATE TABLE `asignacion` (
  ...
  `grupo` varchar(36) DEFAULT NULL,
  `tipo` varchar(15) DEFAULT NULL,
  `dia_semana` varchar(15) NOT NULL,
  `hora_inicio` time NOT NULL,
  `hora_fin` time NOT NULL,
  PRIMARY KEY (`id_asignacion`),
  KEY `fk_asignacion_profesor_idx` (`id_profesor`),
  KEY `fk_asignacion_unidad_idx` (`id_unidad`),
  CONSTRAINT `fk_asignacion_profesor` FOREIGN KEY (`id_profesor`) REFERENCES `profesor` (`id_profesor`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_asignacion_unidad` FOREIGN KEY (`id_unidad`) REFERENCES `unidad_aprendizaje` (`id_unidad`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE `unidad_aprendizaje` (
  ...
  `nombre` varchar(50) NOT NULL,
  `horas_clase` tinyint NOT NULL,
  `horas_taller` tinyint NOT NULL,
  `horas_laboratorio` tinyint NOT NULL,
  PRIMARY KEY (`id_unidad`),
  CONSTRAINT `unidad_aprendizaje_chk_1` CHECK ((`horas_clase` between 0 and 4)),
  CONSTRAINT `unidad_aprendizaje_chk_2` CHECK ((`horas_taller` between 0 and 4)),
  CONSTRAINT `unidad_aprendizaje_chk_3` CHECK ((`horas_laboratorio` between 0 and 4))
...
CREATE TABLE `usuario` (
  ...
  KEY `fk_usuario_alumno_idx` (`idAlumno`),
  CONSTRAINT `fk_usuario_alumno` FOREIGN KEY (`idAlumno`) REFERENCES `alumno` (`idAlumno`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8mb3;
/*!40101 SET character_set_client = @saved_cs_client */;
/*!40103 SET TIME_ZONE=@OLD_TIME_ZONE */;
